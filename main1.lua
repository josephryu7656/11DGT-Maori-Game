...
mapData = love.image.newImageData("map.png")
    ...
    player.animations.jump = anim8.newAnimation(player.grid('1-5', 5), 0.25)
...
function getFloorYAt(x)
    -- Convert world x to image x
    local imgX = math.floor(x / 3) -- since your map is scaled by 3
    if imgX < 0 or imgX >= mapData:getWidth() then
        return love.graphics.getHeight() -- no floor
    end
    ...
    for imgY = 0, mapData:getHeight() - 1 do
        ...
        if a > 0 then -- non-transparent = ground
            return mapY + imgY * 3
        end
    end

    return love.graphics.getHeight() -- if nothing solid
end
...
function love.update(dt) -- It's called automatically by the engine for every frame of the game.
    ...
    -- Find floor at player’s center X
